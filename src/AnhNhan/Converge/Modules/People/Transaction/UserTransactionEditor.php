...
final class UserTransactionEditor extends TransactionEditor
{
    ...
    {
        static $entityRole = 'AnhNhan\Converge\Modules\People\Storage\Role';
        static $entityEmail = 'AnhNhan\Converge\Modules\People\Storage\Email';
        ...
        if (in_array($transaction->type, array(UserTransaction::TYPE_ADD_ROLE, UserTransaction::TYPE_REMOVE_ROLE))) {
            ...
            $role = id(new \AnhNhan\Converge\Modules\People\Query\RoleQuery($this->em()))->retrieveRole($roleId);
            if (!$role) {
